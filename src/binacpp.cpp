	...
	Json::Value &json_result )
{
	...
	querystring.append( to_string__( limit ) );
...
)
{
	...
	if ( startTime != 0 && endTime != 0 ) {
		...
		querystring.append( to_string__( startTime ) );

		querystring.append("&endTime=");
		querystring.append( to_string__( endTime ) );

	} else {
		querystring.append("&fromId=");
		querystring.append( to_string__( fromId ) );
...
BinaCPP::get_account( long recvWindow,  Json::Value &json_result )
{
	...
	querystring.append( to_string__( get_current_ms_epoch() ) );

	if ( recvWindow > 0 ) {
		querystring.append("&recvWindow=");
		querystring.append( to_string__( recvWindow ) );
	}
...

{
	...
	if ( orderId > 0 ) {
		...
		querystring.append( to_string__( orderId ) );
	}
	...
	Json::Value &json_result )
{
	...
	post_data.append( to_string__( quantity) );

	post_data.append("&price=");
	post_data.append( to_string__( price) );
	...
	if ( stopPrice > 0.0 ) {
		...
		post_data.append( to_string__( stopPrice ) );
	}

	if ( icebergQty > 0.0 ) {
		post_data.append("&icebergQty=");
		post_data.append( to_string__( icebergQty ) );
	}

	if ( recvWindow > 0 ) {
		post_data.append("&recvWindow=");
		post_data.append( to_string__( recvWindow) );
	}


	post_data.append("&timestamp=");
	post_data.append( to_string__( get_current_ms_epoch() ) );
	...
	Json::Value &json_result )
{
	...
	if ( orderId > 0 ) {
		...
		post_data.append( to_string__( orderId ) );
	}
	...
	Json::Value &json_result )
{
	...
	post_data.append( to_string__( amount ));
	...
	Json::Value &json_result )
{
	...
	if ( status > 0 ) {
		...
		querystring.append( to_string__( status ) );
	}
